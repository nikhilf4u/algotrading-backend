...
public interface ContractDao  extends JpaRepository<Contract,Integer> {
    @Query(value = "SELECT price as price,open_interest as openInterest,strike_price as strikePrice,time as time,value as value,atp as atp FROM contract WHERE index_type=?1 and strike_price=?2 and option_type=?3 and timestamp like ?4",nativeQuery = true)
    List<ContractDto> getContracts(String index,String strikePrice,String optionType,String currentDate);
